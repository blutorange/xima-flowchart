...
<%@page import="de.xima.cmn.criteria.QueryCriteriaManager"%>
<%@page import="de.xima.fc.entities.Mandant"%>
<%@page import="de.xima.fc.entities.Postfach"%>
...
<%@page import="de.xima.fc.entities.Benutzer"%>
...
<%!
  public String addHTMLResource(Integer id, JSONObject htmlResources, List<Textbaustein> textbausteinListe) {
  ...
  public String addMailBoxResource(Postfach postfach, JSONObject mailboxResources, JSONObject userGroupsResources, JSONObject userResources, Locale locale) {
    String ximaId = "de.xima.fc.resource.mailbox-" + String.valueOf(postfach.getId());
    try {
      if (!mailboxResources.has(ximaId)){
        JSONObject mailboxResource = new JSONObject();
        JSONArray userGroupsJsonArray = new JSONArray();
        mailboxResources.put(ximaId,mailboxResource);
        mailboxResource.put("id",ximaId);
        mailboxResource.put("name",postfach.getName());
        mailboxResource.put("description",postfach.getBeschreibung());
        mailboxResource.put("deletable",postfach.isDeletable());
        if (postfach.getBenutzerGruppen()!=null){
          for (BenutzerGruppe b : postfach.getBenutzerGruppen()){
            userGroupsJsonArray.put(addUserGroupResource(b,userGroupsResources,mailboxResources,userResources,locale));
          }
        }
        mailboxResource.put("restrictToUserGroup",userGroupsJsonArray);
      }
    }
    catch (Exception e) {
      e.printStackTrace();
  ...
  public String addMailBoxResource(Integer id, JSONObject mailboxResources, List<Postfach> postfachListe, JSONObject userGroupsResources, JSONObject userResources, Locale locale) {
    String ximaId = "de.xima.fc.resource.mailbox-" + String.valueOf(id);
    try {
      if (!mailboxResources.has(ximaId)){
        for (Postfach postfach : postfachListe) {
          if (postfach.getId() == id) {
            addMailBoxResource(postfach, mailboxResources, userGroupsResources, userResources, locale);
            break;
          }
        }
      }
  ...
  public String addURLResource(Integer id, JSONObject urlResources, List<Textbaustein> textbausteinListe) {
    String ximaId = "de.xima.fc.resource.url-" + String.valueOf(id);
    try {
      if (!urlResources.has(ximaId)){
        for (Textbaustein textbaustein : textbausteinListe) {
          if (textbaustein.getId() == id) {
            JSONObject urlResource = new JSONObject();
            urlResource.put("id",ximaId);
            urlResource.put("name",textbaustein.getName());
            urlResource.put("url",textbaustein.getBeschreibung());
            urlResources.put(ximaId,urlResource);
            break;
          }
        }
      }
  ...
  public String addXSLResource(Integer id, JSONObject xslResources, List<Textbaustein> textbausteinListe) {
  ...
  public String addPluginResource(Class clazz, JSONObject pluginResources) {    
    ...
    try {
      ...
      if (clazz!=null){
        pluginResource.put("id",ximaId);
        pluginResource.put("class",clazz);
      }
      else {
        pluginResource.put("id",ximaId);
        pluginResource.put("class","de.xima.fc.null");
      }
      pluginResources.put(ximaId,pluginResource);
    }
    catch (Exception e) {
      e.printStackTrace();
    }
    ...
    try {
      ...
      switch (cond.getExecCondition()) {
        ...
        case FORMVALUE_MATCH:
          ...
          putStringToJson(condDetails,"conditionLHS",cond.getFormFieldName());
          switch (cond.getWeiterverarbeitung()) {
            case ABBRUCH:
              condDetails.put("nextAction", "STOP");
              break;
  ...
  public void putStringToJson(JSONObject j, String k, String v) {
    try {
      if (v!=null){
        j.put(k,v);
      }
      else {
        j.put(k,JSONObject.NULL);
      }
    }
    catch (Exception e) {
      e.printStackTrace();                                   
  ...
  public JSONArray getParamMapJsonArray(Map<String,String> parameterMap) {
    ...
    try {
      if (parameterMap!=null) {
        for (Map.Entry<String,String> entry : parameterMap.entrySet()) {
          ...
          putStringToJson(parameter,"key",key);
          putStringToJson(parameter,"value",value);
          array.put(parameter);
        }    
      }
    }
  ...
  public JSONObject getAktionJson(Aktion action, JSONObject imageFont, Status status, JSONArray transitionsJsonArray, Locale locale, Map<String,Integer> imageFontMap, Map<String,String> imageFontValues, JSONObject htmlJsonObject, JSONObject xslJsonObject, JSONObject pluginJsonObject, JSONObject mailboxJsonObject, JSONObject userGroupJsonObject, JSONObject userJsonObject, JSONObject urlJsonObject, List<Textbaustein> textbausteinListe, List<Postfach> postfachListe, Mandant mandant) {
    JSONObject j = new JSONObject();
    ...
    try {
      ...
      switch (procType) {
        case ADD_FILE_TO_VORGANG:
          ...
          break;
        case CALLBACK:
          VerarbeitungCallback procCallback = (VerarbeitungCallback)(proc);
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 1);
          DetailsJsonObject.put("callbackName",procCallback.getCallbackHandlerName());
          break;
        case CHANGE_STATUS:
        ...
        case COMPRESS_AS_ZIP:
          VerarbeitungCompressAsZip procCompressAsZip = (VerarbeitungCompressAsZip)(proc);    
          PropJsonObject.put("de.xima.fc.action.requiresFile", true);
          PropJsonObject.put("de.xima.fc.action.type", 10);
          DetailsJsonObject.put("saveAs",procCompressAsZip.getExportName());
          DetailsJsonObject.put("loadFrom", getFileActionIds(procCompressAsZip.getAktionsIdErgebnisse()));
          break;
        case COPY_TO_POSTFACH:
          VerarbeitungCopyToPostfach procCopyToMailBox = (VerarbeitungCopyToPostfach)(proc);
          Integer mailboxId = procCopyToMailBox.getPostfachId();
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 21);
          DetailsJsonObject.put("keepStatus",procCopyToMailBox.isStatusBeibehalten());
          if (mailboxId>=0) {
            DetailsJsonObject.put("targetMailBox",addMailBoxResource(mailboxId,mailboxJsonObject,postfachListe,userGroupJsonObject,userJsonObject,locale));
          }
          else {
            DetailsJsonObject.put("targetMailBox",JSONObject.NULL);
          }
          break;
        case COPY_TO_STATUS:
        ...
        case DB_SQL_STATEMENT:
          ...
          putStringToJson(DetailsJsonObject,"sqlStatement",procSql.getQuery());
          break;
        case DELETE_VORGANG:
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
        ...
        case EMAIL:
          ...
          putStringToJson(DetailsJsonObject,"headerTo",procEMail.getTo());
        ...
        case EXTERNAL_RESOURCE:
          ...
          putStringToJson(DetailsJsonObject,"resourceURL",procExternalResource.getResource());
          DetailsJsonObject.put("saveAs",procExternalResource.getExportName());
          break;
        case MOVE_TO_POSTFACH:
          VerarbeitungMoveToPostfach procMoveToMailBox = (VerarbeitungMoveToPostfach)(proc);
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 22);
          if (procMoveToMailBox.getPostfachId()>=0) {
            DetailsJsonObject.put("targetMailBox",addMailBoxResource(procMoveToMailBox.getPostfachId(),mailboxJsonObject,postfachListe,userGroupJsonObject,userJsonObject,locale));
          }
          else {
            DetailsJsonObject.put("targetMailBox",JSONObject.NULL);
          }
          break;
        case PDF_FILL:
          VerarbeitungPdf procPdfFilll = (VerarbeitungPdf)(proc);      
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 11);
          putStringToJson(DetailsJsonObject,"sourceTypeDisplayName",procPdfFilll.getRessTypeEnum().getDisplayName(locale));
          putStringToJson(DetailsJsonObject,"saveAs",procPdfFilll.getExportName());
          putStringToJson(DetailsJsonObject,"resourceName",procPdfFilll.getResource());
          switch (procPdfFilll.getRessTypeEnum()) {
            case EXTERNE_RESSOURCE:
                DetailsJsonObject.put("sourceType","EXTERNAL");
              break;
            case FORMULAR:
                DetailsJsonObject.put("sourceType","FORM");
              break;
            case MANDANT:
                DetailsJsonObject.put("sourceType","TENANT");    
              break;
            case PROJEKT:
                DetailsJsonObject.put("sourceType","PROJECT");    
              break;
          }
          break;
        case PERSISTENCE_EXPORT:
          ...
          putStringToJson(DetailsJsonObject,"saveAs",procExportPersistence.getExportFileName());
          break;
        case PLUGIN:
          VerarbeitungPlugin procPlugin = (VerarbeitungPlugin)(proc);    
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 13);
          putStringToJson(DetailsJsonObject,"name",procPlugin.getSelectedPlugin());
          DetailsJsonObject.put("parameters",getParamMapJsonArray(procPlugin.getParamMap()));
          DetailsJsonObject.put("plugin",addPluginResource(procPlugin.getPluginClass(mandant),pluginJsonObject));
          break;
        case POST:
          VerarbeitungPost procPost = (VerarbeitungPost)(proc);       
          JSONArray parameterJsonArray2 = new JSONArray();
          xslID = procPost.getTextbausteinId();
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 12);
          putStringToJson(DetailsJsonObject,"targetURL",procPost.getPostURL());
          DetailsJsonObject.put("allValues",procPost.isIncludeFD());
          DetailsJsonObject.put("noFiles",procPost.isUseSinglePart());
          putStringToJson(DetailsJsonObject,"requestEncoding",procPost.getInputCharset());
          putStringToJson(DetailsJsonObject,"responseEncoding",procPost.getOutputCharset());
          if (xslID>=0) {
            DetailsJsonObject.put("xslTemplate",addXSLResource(xslID,xslJsonObject,textbausteinListe));
          }
          else {
            DetailsJsonObject.put("xslTemplate",JSONObject.NULL);    
          }
          DetailsJsonObject.put("urlParameters",getParamMapJsonArray(procPost.getParamMap()));
          break;
        case PROVIDE_UPLOAD:
          VerarbeitungUpload procUpload = (VerarbeitungUpload)(proc);
          JSONArray FilesJsonArray = new JSONArray();
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 17);
          if (procUpload.getUploadFiles()!=null) {
            for (String s : procUpload.getUploadFiles()) {
              FilesJsonArray.put(s);
            }
          }
          DetailsJsonObject.put("uploadElementNames",FilesJsonArray);
          break;
        case REDIRECT:
          VerarbeitungWeiterleitung procRedirect = (VerarbeitungWeiterleitung)(proc);
          Integer urlID = procRedirect.getTextbausteinId();
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 24);
          DetailsJsonObject.put("urlParameters",getParamMapJsonArray(procRedirect.getParamMap()));
          if (urlID>=0) {
            DetailsJsonObject.put("redirectTemplate",addURLResource(urlID,urlJsonObject,textbausteinListe));
          }
          else {
            DetailsJsonObject.put("redirectTemplate",JSONObject.NULL);    
          }
          break;
        ...
        case SAVE_TO_FILESYSTEM:
          VerarbeitungSaveToFilesystem procSaveToFileSystem = (VerarbeitungSaveToFilesystem)(proc);    
          String saveAs = procSaveToFileSystem.getExportDir();
          PropJsonObject.put("de.xima.fc.action.requiresFile", true);
          PropJsonObject.put("de.xima.fc.action.type", 14);
          putStringToJson(DetailsJsonObject,"saveAs",procSaveToFileSystem.getExportDir());
          DetailsJsonObject.put("loadFrom", getFileActionIds(procSaveToFileSystem.getAktionsIdErgebnisse()));
          DetailsJsonObject.put("usePID",procSaveToFileSystem.isUseProzessId());
          DetailsJsonObject.put("useTimeStamp",procSaveToFileSystem.isUseTimestamp());
          break;
        ...
        case SHOW_TEMPLATE:
          ...
          if (htmlId>=0) {
            DetailsJsonObject.put("targetTemplate",addHTMLResource(htmlId,htmlJsonObject,textbausteinListe));
          }
          else {
            DetailsJsonObject.put("targetTemplate",JSONObject.NULL);
          }
          break;
        case STOP:
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          ...
          break;
        case WORD_FILL:
          VerarbeitungWord procWordFill = (VerarbeitungWord)(proc);    
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 25);
          putStringToJson(DetailsJsonObject,"sourceTypeDisplayName",procWordFill.getRessTypeEnum().getDisplayName(locale));
          putStringToJson(DetailsJsonObject,"saveAs",procWordFill.getExportName());
          putStringToJson(DetailsJsonObject,"resourceName",procWordFill.getResource());
          switch (procWordFill.getRessTypeEnum()) {
            case EXTERNE_RESSOURCE:
                DetailsJsonObject.put("sourceType","EXTERNAL");
              break;
            case FORMULAR:
          ...
          break;
        case WRITE_TO_FILE:
          VerarbeitungWriteToFile procWriteToFile = (VerarbeitungWriteToFile)(proc);    
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 16);
          putStringToJson(DetailsJsonObject,"saveAs",procWriteToFile.getExportName());
          putStringToJson(DetailsJsonObject,"textContent",procWriteToFile.getFileContent());
          break;
        case WRITE_TO_FORM:
          VerarbeitungWriteToForm procWriteToForm = (VerarbeitungWriteToForm)(proc);    
          PropJsonObject.put("de.xima.fc.action.requiresFile", false);
          PropJsonObject.put("de.xima.fc.action.type", 9);
          DetailsJsonObject.put("formChanges",getParamMapJsonArray(procWriteToForm.getParamMap()));
          break;
        case XML_EXPORT:
          ...
          putStringToJson(DetailsJsonObject,"saveAs",procExportXml.getExportFileName());
          ...
          break;
        case XML_TO_FD:
          VerarbeitungXmlToFd procXmlToForm = (VerarbeitungXmlToFd)(proc);
          PropJsonObject.put("de.xima.fc.action.requiresFile", true);
          PropJsonObject.put("de.xima.fc.action.type", 26);
          DetailsJsonObject.put("sourceAction","de.xima.fc.action-" + procXmlToForm.getSourceAktionId());
          DetailsJsonObject.put("xPath",getParamMapJsonArray(procXmlToForm.getParamMap()));
          if (procXmlToForm.getTextbausteinId()>=0) {
            DetailsJsonObject.put("xslTemplate",addXSLResource(procXmlToForm.getTextbausteinId(),xslJsonObject,textbausteinListe));
          }
          else {
            DetailsJsonObject.put("xslTemplate",JSONObject.NULL);    
          }
  ...
  public String addUserResource(Benutzer user, JSONObject userResources, JSONObject userGroupsResources, JSONObject mailboxResources, Locale locale) {
    String userID = "de.xima.fc.resource.user-" + String.valueOf(user.getId());
    try {
      JSONObject userJsonObject = new JSONObject();
      if (!userResources.has(userID)) {
        JSONArray userGroupJsonArray = new JSONArray();
        userResources.put(userID, userJsonObject);
        putStringToJson(userJsonObject,"id", userID);
        putStringToJson(userJsonObject,"name", user.getName());
        putStringToJson(userJsonObject,"oldPassword", null);
        putStringToJson(userJsonObject,"clearTextPassword", null);
        putStringToJson(userJsonObject,"eMail", user.getEmail());
        putStringToJson(userJsonObject,"formOfAddressDisplayName", user.getAnrede().getDisplayName(locale));
        putStringToJson(userJsonObject,"statusDisplayName", user.getBenutzerstatus().getDisplayName(locale));
        putStringToJson(userJsonObject,"fullName", user.getFullName());
        putStringToJson(userJsonObject,"loginName", user.getLoginName());
        putStringToJson(userJsonObject,"familyName", user.getNachname());
        putStringToJson(userJsonObject,"password", null);
        putStringToJson(userJsonObject,"phone", user.getTelefon());
        putStringToJson(userJsonObject,"personalName", user.getVorname());
        putStringToJson(userJsonObject,"systemID", String.valueOf(user.getId()));
        userJsonObject.put("userGroups", userGroupJsonArray);
        for (BenutzerGruppe bg : user.getBenutzerGruppen()) {
          userGroupJsonArray.put(addUserGroupResource(bg,userGroupsResources,mailboxResources,userResources,locale));
        }
        switch (user.getAnrede()) {
          case FRAU:
            putStringToJson(userJsonObject,"formOfAddress", "LADY");
            break;
          case HERR:
            putStringToJson(userJsonObject,"formOfAddress", "SIR");
            break;
          case KEINE_ANGABE:
            putStringToJson(userJsonObject,"formOfAddress", "NONE_GIVEN");
            break;
        }
        switch (user.getBenutzertyp()) {
          case INTERN:
            putStringToJson(userJsonObject,"userType", "INTERNAL");
            break;
          case LDAP:
            putStringToJson(userJsonObject,"userType", "LDAP");
            break;
        }
        switch (user.getBenutzerstatus()) {
          case ACTIVE:
            putStringToJson(userJsonObject,"status", "ACTIVE");
            break;
          case DELETED:
            putStringToJson(userJsonObject,"status", "DELETED");
            break;
          case LOCKED:
            putStringToJson(userJsonObject,"status", "LOCKED");
            break;
        }
      }
    }
  ...
  public String addUserGroupResource(BenutzerGruppe userGroup, JSONObject userGroupsResources, JSONObject mailboxResources, JSONObject userResources, Locale locale) {
    ...
    try {
      ...
      if (!userGroupsResources.has(userGroupID)) {
        userGroupsResources.put(userGroupID, userGroupJsonObject);
        ...
        for (Postfach p : userGroup.getPostfaecher()){
          mailboxAccess.put(addMailBoxResource(p,mailboxResources,userGroupsResources,userResources,locale));
        }
        for (Benutzer b : userGroup.getBenutzer()){
          users.put(addUserResource(b,userResources,userGroupsResources,mailboxResources,locale));
        }
  ...
  public JSONArray getRestrictionsFrom(List<BenutzerGruppe> userGroups, JSONObject userGroupsResources, JSONObject mailboxResources, JSONObject userResources, Locale locale) {
    JSONArray j = new JSONArray();
    try {
      for (BenutzerGruppe b : userGroups) {
        j.put(addUserGroupResource(b, userGroupsResources, mailboxResources, userResources, locale));
      }
    }
    catch (Exception e) {
      e.printStackTrace();
  ...
  public JSONObject getStatusJson(Status status, JSONObject imageFont, JSONArray transitionsJsonArray, Locale locale, Map<String,Integer> imageFontMap, Map<String,String> imageFontValues, JSONObject htmlJsonObject, JSONObject xslJsonObject, JSONObject pluginJsonObject, JSONObject mailboxJsonObject, JSONObject userGroupJsonObject, JSONObject userJsonObject, JSONObject urlJsonObject, List<Textbaustein> textbausteinListe, List<Postfach> postfachListe, Mandant mandant) {
    ...
    try {
      ...
      restrictionsFromJsonArray = getRestrictionsFrom(restrictionsFrom, userGroupJsonObject, mailboxJsonObject, userJsonObject, locale);
      ...
      for (Aktion a : actions) {
        ActionArray.put(getAktionJson(a, imageFont, status, transitionsJsonArray, locale, imageFontMap, imageFontValues, htmlJsonObject, xslJsonObject, pluginJsonObject, mailboxJsonObject, userGroupJsonObject, userJsonObject, urlJsonObject, textbausteinListe, postfachListe, mandant));
      }
      ...
      if (timerTargetStatus != null) {
        ...
        PropObject.put("de.xima.fc.status.timedTransitionTarget", JSONObject.NULL);
      }
...
<%
  ...
  try {
    ...
    QueryCriteriaManager qcm = new QueryCriteriaManager();

    Projekt projekt = DaoProvider.PROJEKT_DAO.read(ec, projektId);
    Mandant mandant = projekt.getMandant();
    List<Status> statusListe = DaoProvider.STATUS_DAO.getAllByProjekt(ec, projekt);
    List<Textbaustein> textbausteinListe = DaoProvider.TEXTBAUSTEIN_DAO.getAll(ec, mandant);
    List<Postfach> postfachListe = DaoProvider.POSTFACH_DAO.all(ec,qcm);
    ...
    for (Status s : statusListe) {
      statusJsonArr.put(getStatusJson(s, imageFontJsonObject, edgesJsonArr, locale, imageFontMap, imageFontValues, htmlJsonObject, xslJsonObject, pluginJsonObject, mailboxJsonObject, userGroupJsonObject, userJsonObject, urlJsonObject, textbausteinListe, postfachListe, mandant));
    }
    ...
    resourcesJsonObject.put("user", userJsonObject);
